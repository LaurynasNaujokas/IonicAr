...
export class KokkerService {
  private kokkers: Kokker[] = [
    {
      kokkerId : '1',
      firstName: 'Louisa',
      lastName: 'Lorang',
...
// tslint:disable-next-line: max-line-length
  ...
  // Getting the list of all kokkers
  getAllKokkers() {
    return [...this.kokkers];
  }

  // finding a kokker with specifc id  to get data in kokker-details page
  // or share elsewhere
  getKokker(kokkerId: string){
    return {
      ...this.kokkers.find(kokker => {
      return kokker.kokkerId === kokkerId;
    })};
  }
